...
class CarSpyGame:
    """I Spy with my little eye... a car! - A fun detection game"""
    ...
    def process_game_frame(self, frame, detections):
        ...
        for det in detections:
            ...
            if class_name.lower() == self.target_car.lower():
                ...
                # Increment score while target is visible
                self.score += 1
    ...
    def add_sparkle_effect(self, frame, x1, y1, x2, y2):
        ...
        for _ in range(5):
            ...
            # Draw sparkle
            cv2.circle(frame, (sparkle_x, sparkle_y), 3, (0, 255, 255), -1)
            cv2.circle(frame, (sparkle_x, sparkle_y), 5, (255, 255, 255), 1)
    
    def set_new_target(self):
        """Set a new target car to find."""
        self.target_car = random.choice(self.car_types)
        self.targets_found = 0
        
        # Show fun fact
        fact = random.choice(self.fun_facts)
        print(f"\n🎯 New target: {self.target_car}")
        print(f"💡 Fun fact: {fact}")
    
    def level_up(self):
        """Level up the game."""
        self.level += 1
        print(f"\n🎉 LEVEL UP! Now at level {self.level}!")
        print(f"🎁 Bonus: Time extended by 30 seconds!")
        self.time_limit += 30
